...
EOT
    ...
    {
        return [
            ['file', InputArgument::OPTIONAL, 'Json file to use', './satis.json'],
            ['output-dir', InputArgument::OPTIONAL, 'Location where to output built files', null],
            ['packages', InputArgument::IS_ARRAY | InputArgument::OPTIONAL, 'Packages that should be built. If not provided, all packages are built.', null],
        ];
    }
    ...
    {
        return [
            ['repository-url', null, InputOption::VALUE_OPTIONAL, 'Only update the repository at given url', null],
            ['repository-strict', null, InputOption::VALUE_NONE, 'Also apply the repository filter when resolving dependencies', null],
            ['no-html-output', null, InputOption::VALUE_NONE, 'Turn off HTML view', null],
            ['skip-errors', null, InputOption::VALUE_NONE, 'Skip Download or Archive errors', null],
            ['stats', null, InputOption::VALUE_NONE, 'Display the download progress bar', null],
        ];
    }
    ...
    private function check($configFile)
    {
        ...
        if (null === $result) {
            ...
            $schemaFile = base_path('satis-schema.json');
